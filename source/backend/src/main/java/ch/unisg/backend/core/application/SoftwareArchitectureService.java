  ...
  public void create(CreateArchitectureRequirement command) {
    switch (command.type()) {
      ...
      case "nonFunctionalRequirement" ->
        nonFunctionalRequirementPort.create(NonFunctionalRequirement.create(command.id(), command.title()));
      case "intention" -> intentionPort.create(Intention.create(command.id(), command.title()));
      case "architecturePrinciple" ->
        architecturePrinciplePort.create(ArchitecturePrinciple.create(command.id(), command.title()));
    }
  }
