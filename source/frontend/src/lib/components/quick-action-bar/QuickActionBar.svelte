<script lang="ts">
  ...
  import CreateArchitecturalRequirementsForm from "$lib/components/form/akm/CreateArchitecturalRequirementsForm.svelte";
...
<div class="w3-margin w3-round-xlarge quick-action-bar">
  <!-- Quick action items -->
  <QuickActionItemBar form="createSystem" icon="S+" tooltip="Create system" />
  <QuickActionItemBar
    form="createSystemElement"
    icon="SE+"
    tooltip="Create system element"
  />
  <QuickActionItemBar
    form="createArchitecturalKnowledge"
    icon="AK+"
    tooltip="Create architectural knowledge"
  />
  <QuickActionItemBar
    form="updateArchitecturalKnowledge"
    icon="AK~"
    tooltip="Update architectural knowledge"
  />
  <QuickActionItemBar
    form="createArchitecturalRequirements"
    icon="AR+"
    tooltip="Create architectural requirements"
  />
  <QuickActionItemBar form="askAgent" icon="AI" tooltip="Ask agent" />

  <!-- Switch views -->
  <div style="position:fixed; bottom:40px">
    <a target="_blank" href="http://localhost:7474" class="w3-button">AKM</a>
  </div>
</div>
...
<div
  id="div-forms"
  class="w3-sidebar form-style w3-right w3-bar-block w3-animate-right padding-16 w3-round-xlarge w3-margin"
  style="display: {formManager.overlay ? 'block' : 'none'};width: 600px;"
>
  <button
