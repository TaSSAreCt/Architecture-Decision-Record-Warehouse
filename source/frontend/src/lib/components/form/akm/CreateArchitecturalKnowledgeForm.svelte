...
<div>
  ...
  {#if issue}
    ...
    <form
      id="alternativeForm"
      method="POST"
      action="actions/ad/alternative?/createAlternativeWithIssueAndArchitecturalRequirements"
      use:enhance={({ formData, formElement }) => {
        ...
        const influenceIds = Array.from(
          { length: selectedArchitecturalRequirements.length },
          () => crypto.randomUUID(),
        );

        formData.set("influenceIds", influenceIds.join(","));
        formData.set(
          "selectedArchitecturalRequirements",
          selectedArchitecturalRequirements.join(","),
        );
        formData.set("values", values.join(","));

        return async ({ result }) => {
          selectedArchitecturalRequirements.forEach(
            (architectureRequirementId, index) => {
              let architectureRequirement: ArchitectureRequirement | undefined =
                architectureRequirementFlatMap.find(
                  (item) => item.id === architectureRequirementId,
                );
    ...
    >
      ...
      <div class="" style="width: 400px">
        <label for="architecturalRequirements"
          >Choose architectural requirements:</label
        >
        <select
          name="architecturalRequirements"
          id="architecturalRequirements"
          bind:value={selectedArchitecturalRequirements}
          multiple
        >
          {#each architectureRequirementFlatMap as architectureRequirement}
            <option value={architectureRequirement.id}
              >{architectureRequirement.title}</option
            >
          {/each}
        </select>
      </div>
    ...
    {#each issue.alternativeList as alternative}
      <div class="w3-container">
        <p>New alternative: {alternative.title}</p>
      </div>
    {/each}
  {:else if !issue}
    <form
      id="issueForm"
      method="POST"
      action="actions/ad/issue?/createIssue"
      use:enhance={({ formData, formElement }) => {
        issue = Issue.create();
