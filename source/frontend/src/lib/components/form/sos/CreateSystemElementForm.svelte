...
<div class="">
  ...
  <form
    method="post"
    action="actions/sos/system-elements?/createSystemElement"
    use:enhance={({ formData }) => {
      ...
      systemElement.isCyber = formData.get("isCyber") === "true";

      formData.set("id", systemElement.id);

      let system: System =
        getSystemOfSystems(cpsos, String(formData.get("systemId"))) ??
        System.create();
