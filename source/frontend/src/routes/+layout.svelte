<script lang="ts">
  ...
  import { fromArchitecturalRequirementsResponseDto } from "$lib/dto/response/entity/ArchitecturalRequirementsResponseDto";
  ...
  const architecturalKnowledge: ArchitecturalKnowledge =
    ArchitecturalKnowledge.create();
  architecturalKnowledge.issueList = fromIssueListResponseDto(
    data.architecturalKnowledge.issueList,
  );
  architecturalKnowledge.architecturalRequirements =
    fromArchitecturalRequirementsResponseDto(
      data.architecturalKnowledge.architectureRequirementList,
    );
...
<div id="background" class="">
  ...
  <div
    class="w3-overlay"
    style="display: {formManager.overlay ? 'block' : 'none'}; z-index: 4;"
    id="myOverlay"
  ></div>
